...
Module.register('MMM-Face-Reco-DNN', {
  ...
  // ----------------------------------------------------------------------------------------------------
  login_user: function(name, age, gender) {
    var self = this;
    var thisUserClasses;
    var existingClasses;
    var newClassList;
    ...
    Log.log('Logged in user:' + name + ' age: ' + age + ' gender: ' + gender);
    // user not currently logged in so add them to the list of logged in users
    this.users.push(name);
    this.ages.push(age);
    this.genders.push(gender);
    this.config.debug && Log.log('User list after login:' + this.users + ' with ages: ' + this.ages + ' and genders: ' + this.genders);
  ...
  // ----------------------------------------------------------------------------------------------------
  logout_user: function(name, age, gender) {
    var self = this;
  ...
  socketNotificationReceived: function(notification, payload) {
    ...
    if (payload.action === 'login') {
        ...
        if (user != null) {
          ...
          if (this.users.length === 0 || this.users.length < this.config.multiUser) {
            ...
            if (!this.users.includes(user)) {
              ...
              this.login_user(user, payload.ages[loginCount], payload.genders[loginCount]);
      ...
      if (loginCount>0) {
         ...
         this.sendNotification('USERS_LOGIN', {
          users: payload.users,
          ages: payload.ages,
        ...
        if (user != null) {
          ...
          if (this.users.includes(user)) {
            ...
            this.timouts[user] = setTimeout(function() {
              ...
              self.sendNotification('USERS_LOGOUT_MODULES', user, payload.ages[logoutCount], payload.genders[logoutCount]);
              self.logout_user(user, payload.ages[logoutCount], payload.genders[logoutCount]);
              logoutCount++;
      ...
      if (logoutCount>0) {
         ...
         this.sendNotification('USERS_LOGOUT', payload.users, payload.ages, payload.genders);
      }
    }
  },
  ...
  notificationReceived: function(notification, payload, sender) {
    ...
    if (notification === 'GET_LOGGED_IN_USERS') {
      ...
      this.sendNotification('LOGGED_IN_USERS', this.users, this.ages, this.genders);
    }
  },
});
