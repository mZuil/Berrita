...
- El archivo tools\encode.py contiene la codificación que permite el reconocimiento facial
- El archivo tools\facerecognition.py ha sido modificado para captar los metadatos del usuario (edad y género) a través de la imagen captada con la cámara, y enviarla también al resto de módulos.
- El archivo tools\facerecognition_prueba.py es otra aproximación que se hizo del código desarrollado en tools\facerecognition.py, en un intento de optimizar el proceso.
- La carpeta metadata-detection contiene los modelos y scripts utilizados para poder reconocer tanto la edad como el género del usuario frente al espejo.
- Los archivos node_helper.js y MMM-Face-Reco-DNN.js fueron modificados de tal forma que se pasase al resto de módulos también la información de la edad y el género.
